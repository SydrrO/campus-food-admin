...
async def login(payload: UserLoginIn, db: Session = Depends(get_db)):
    """微信登录：real 模式调用 code2session，mock 模式生成测试 openid。"""
    try:
        if is_wechat_pay_real_mode():
            openid = await exchange_code_for_openid(payload.code)
        else:
            try:
                pinned = db.query(User).filter(User.openid == "dev-pinned-user").first()
                openid = "dev-pinned-user" if pinned else build_mock_openid(payload.code)
            except Exception:
