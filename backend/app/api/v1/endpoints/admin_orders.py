...
def _build_order_record_text(order: Order, items: list[OrderItem], db: Session | None = None) -> str:
    ...
    # Resolve merchant codes for each item
    if db and items:
        dish_ids = {item.dish_id for item in items}
        dish_code_map = {
            dish_id: code
            for dish_id, code in db.query(Dish.id, Dish.merchant_code)
    ...
    # Group items by merchant code
    groups: dict[str, list[OrderItem]] = {}
    for item in items:
        code = dish_code_map.get(item.dish_id) or ""
        label = MERCHANT_LABELS.get(code, code or "未分类")
...
def _serialize_order_detail(order: Order, items: list[OrderItem], db: Session) -> AdminOrderDetailOut:
    ...
    return AdminOrderDetailOut(
        **summary.model_dump(),
        ...
        record_text=_build_order_record_text(order, items, db),
        items=[
            OrderItemOut(
...
async def get_order_detail(
    ...
    return ResponseModel(data=_serialize_order_detail(order, items, db))
