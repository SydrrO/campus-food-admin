...
def _serialize_order_summary(order: Order, timeout_minutes: int | None = None) -> OrderSummaryOut:
    ...
    return OrderSummaryOut(
        ...
        total_amount=str(order.total_amount),
        delivery_fee=str(order.delivery_fee),
        discount_amount=str(getattr(order, "discount_amount", Decimal("0.00")) or Decimal("0.00")),
...
async def list_orders(
    ...
    return ResponseModel(data=[_serialize_order_summary(order, timeout_minutes) for order in orders])
